class Build
  def Build.configure(config, settings)
    ...
    config.vm.provision "shell" do |s|
       s.inline = "cd /var/www/redisworker && sudo cp ./redisworker /etc/nginx/sites-available/redisworker && sudo ln -s /etc/nginx/sites-available/redisworker  /etc/nginx/sites-enabled/redisworker && sudo service nginx restart"
    end
  end
end